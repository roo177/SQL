...
CREATE OR REPLACE VIEW public.q_cb_curr_escalation_rates
 ...
 SELECT q_cb_active_months_res_qty.rep_month,
    ...
    q_cb_active_months_res_qty.rs_l4,
        CASE
            WHEN r4_code.currency::text = 'TRY'::text AND r4_code.w_inf_usd <> 0::numeric THEN 1::numeric + q_cb_monthly_curr_rates_inc.rt_usd_try * r4_code.w_inf_usd
            ELSE
            CASE
                WHEN r4_code.currency::text = 'EUR'::text AND r4_code.w_inf_usd <> 0::numeric THEN 1::numeric + q_cb_monthly_curr_rates_inc.rt_usd_eur * r4_code.w_inf_usd
                ELSE 1::numeric
            END
        END AS k_usd,
        CASE
            WHEN r4_code.currency::text = 'TRY'::text AND r4_code.w_inf_eur <> 0::numeric THEN 1::numeric + q_cb_monthly_curr_rates_inc.rt_eur_try * r4_code.w_inf_eur
            ELSE
            CASE
                WHEN r4_code.currency::text = 'USD'::text AND r4_code.w_inf_eur <> 0::numeric THEN 1::numeric + q_cb_monthly_curr_rates_inc.rt_eur_usd * r4_code.w_inf_eur
                ELSE 1::numeric
            END
        END AS k_eur,
    r4_code.w_inf_usd,
