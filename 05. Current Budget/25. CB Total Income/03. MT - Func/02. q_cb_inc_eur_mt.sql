-- FUNCTION: public.q_cb_inc_eur_mt()

DROP FUNCTION IF EXISTS public.q_cb_inc_eur_mt();

CREATE OR REPLACE FUNCTION public.q_cb_inc_eur_mt(
	)
    RETURNS boolean
    LANGUAGE 'plpgsql'
    COST 100
...
-- Table: public.t_cb_inc_eur_st

-- DROP TABLE IF EXISTS public.t_cb_inc_eur_st;
...
--ALTER TABLE IF EXISTS public.t_cb_inc_eur_st
--    OWNER to ictasadmin;

Raise notice 'Deleting existing data';
Delete from t_cb_inc_eur_st;
Raise notice 'Appending new data';

Insert into t_cb_inc_eur_st

Select 
q_cb_inc_eur.rep_month, 
q_cb_inc_eur.pc, 
l_1 || '.' || l_2 || '.' || l_3 || '.' || l_4 || '.' || l_5 || '.' || l_6 as j_code, 
c2_code.desc_tr_l2, 
c3_code.desc_tr_l3, 
q_cb_inc_eur.month, 
sum(q_cb_inc_eur.eur_income) as eur_income, 
q_cb_inc_eur.up_curr_conv, 
q_cb_inc_eur.l_1, 
q_cb_inc_eur.l_2, 
q_cb_inc_eur.l_3, 
q_cb_inc_eur.l_4, 
...
q_cb_inc_eur.l_6, 
c6_code.desc_tr_l6, 
q_cb_inc_eur.key_r_pc_l6
from (q_cb_inc_eur left join (c2_code right join c3_code on (c2_code.c_l1 = c3_code.c_l1) and (c2_code.c_l2 = c3_code.c_l2)) on (q_cb_inc_eur.l_3 = c3_code.c_l3) and 
(q_cb_inc_eur.l_2 = c3_code.c_l2) and (q_cb_inc_eur.l_1 = c3_code.c_l1)) left join c6_code on (q_cb_inc_eur.l_6 = c6_code.c_l6) and (q_cb_inc_eur.l_5 = c6_code.c_l5) and 
(q_cb_inc_eur.l_4 = c6_code.c_l4) and (q_cb_inc_eur.l_3 = c6_code.c_l3) and (q_cb_inc_eur.l_2 = c6_code.c_l2) and (q_cb_inc_eur.l_1 = c6_code.c_l1)
group by 
q_cb_inc_eur.rep_month, 
q_cb_inc_eur.pc, 
...
--ALTER FUNCTION public.q_cb_inc_eur_mt()
--    OWNER TO ictasadmin;
