-- Table: public.t_cb_res_up

-- DROP TABLE IF EXISTS public.t_cb_res_up;

CREATE TABLE IF NOT EXISTS public.t_cb_res_up
(
    ...
    rcode character varying(12) COLLATE pg_catalog."default",
    pkey character varying(30) COLLATE pg_catalog."default" GENERATED ALWAYS AS ((((((((((((rep_month)::text || '-'::text) || (pc)::text) || '-'::text) || (rs_l1)::text) || '.'::text) || (rs_l2)::text) || '.'::text) || (rs_l3)::text) || '.'::text) || (rs_l4)::text)) STORED,
    price_month date,
    CONSTRAINT t_pl_res_up_pkey PRIMARY KEY (rep_month, pc, rs_l1, rs_l2, rs_l3, rs_l4),
    CONSTRAINT r__t_pl_re__r4_code FOREIGN KEY (rs_l1, rs_l2, rs_l4, rs_l3)
        REFERENCES public.r4_code (r_1_code, r_2_code, r_4_code, r_3_code) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    CONSTRAINT r__t_pl_re__t_001_p FOREIGN KEY (pc)
        REFERENCES public.t_001_projects (p_code) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    CONSTRAINT r__t_pl_re__t_rep_m FOREIGN KEY (rep_month)
        REFERENCES public.t_rep_month (rep_month) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE RESTRICT,
    CONSTRAINT rcode_fkey FOREIGN KEY (rcode)
        REFERENCES public.r4_code (code_r4) MATCH SIMPLE
        ON UPDATE CASCADE
        ON DELETE RESTRICT
)
...
ALTER TABLE IF EXISTS public.t_cb_res_up
    OWNER to ictasadmin;