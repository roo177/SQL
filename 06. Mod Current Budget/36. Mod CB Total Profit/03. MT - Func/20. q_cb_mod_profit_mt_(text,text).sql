...
AS $BODY$
    ...
    (
		...
		j_code character varying(50) GENERATED ALWAYS AS ("l_1" || '.' || "l_2" || '.' || "l_3" || '.' || "l_4" || '.' || "l_5" || '.' || "l_6" ) STORED,
		jkey character varying(150) COLLATE pg_catalog."default" GENERATED ALWAYS AS ("rep_month" || '.' || "pc" || '.' || "l_1" || '.' || "l_2" || '.' || "l_3" || '.' || "l_4" || '.' || "l_5" || '.' || 		"l_6"  || '.' ||  extract(DAY FROM "month")::text|| '.' || extract(MONTH FROM "month")::text || '.' || extract(YEAR FROM "month")::text || '.' ||  "user_id" || '.'  || "session_id" ) STORED
...
CREATE OR REPLACE VIEW q_cb_mod_pr_wbs
    ...
    --DROP VIEW IF EXISTS q_cb_mod_profit;
 	...
 	EXECUTE format('CREATE OR REPLACE VIEW q_cb_mod_profit
    AS
