-- FUNCTION: public.q_cb_mod_profit_eur_mt(text, text)

DROP FUNCTION IF EXISTS public.q_cb_mod_profit_eur_mt(text, text);

CREATE OR REPLACE FUNCTION public.q_cb_mod_profit_eur_mt(
	_user_id text,
	_session_id text)
    RETURNS boolean
    LANGUAGE 'plpgsql'
...
CREATE TABLE IF NOT EXISTS t_cb_mod_profit_eur_st
(
...
CREATE OR REPLACE VIEW q_cb_mod_profit_eur
 AS
 SELECT t_cb_mod_profit_st.user_id,
   ...
   FROM t_cb_mod_profit_st
    ...
    LATERAL ( SELECT
                CASE
                    WHEN t_cb_mod_profit_st.curr::text = 'USD'::text THEN t_cb_mod_profit_st.income *  t_cb_mod_mon_curr_rates.r_usd_eur::double precision
                    ELSE
                    CASE
                        WHEN t_cb_mod_profit_st.curr::text = 'TRY'::text THEN t_cb_mod_profit_st.income *  t_cb_mod_mon_curr_rates.r_try_eur::double precision
                        ELSE t_cb_mod_profit_st.income
                    END
                END AS income,
                CASE
                    WHEN t_cb_mod_profit_st.curr::text = 'USD'::text THEN t_cb_mod_profit_st.expense *  t_cb_mod_mon_curr_rates.r_usd_eur::double precision
                    ELSE
                    CASE
                        WHEN t_cb_mod_profit_st.curr::text = 'TRY'::text THEN t_cb_mod_profit_st.expense *  t_cb_mod_mon_curr_rates.r_try_eur::double precision
