-- FUNCTION: q_cb_mod_profit_usd_mt()

DROP FUNCTION IF EXISTS q_cb_mod_profit_usd_mt();

CREATE OR REPLACE FUNCTION q_cb_mod_profit_usd_mt(
	)
    RETURNS boolean
    LANGUAGE 'plpgsql'
    COST 100
...
CREATE TEMPORARY TABLE IF NOT EXISTS  t_cb_mod_profit_usd_st
(
...
DROP VIEW IF EXISTS q_cb_mod_profit_usd;

CREATE TEMPORARY VIEW q_cb_mod_profit_usd
 AS
 SELECT t_cb_mod_profit_st.rep_month,
   ...
   FROM t_cb_mod_profit_st
    ...
    LATERAL ( SELECT
                CASE
                    WHEN t_cb_mod_profit_st.curr::text = 'EUR'::text THEN t_cb_mod_profit_st.income * mon_curr_rates.r_eur_usd::double precision
                    ELSE
                    CASE
                        WHEN t_cb_mod_profit_st.curr::text = 'TRY'::text THEN t_cb_mod_profit_st.income * mon_curr_rates.r_try_usd::double precision
                        ELSE t_cb_mod_profit_st.income
                    END
                END AS income,
                CASE
                    WHEN t_cb_mod_profit_st.curr::text = 'EUR'::text THEN t_cb_mod_profit_st.expense * mon_curr_rates.r_eur_usd::double precision
                    ELSE
                    CASE
                        WHEN t_cb_mod_profit_st.curr::text = 'TRY'::text THEN t_cb_mod_profit_st.expense * mon_curr_rates.r_try_usd::double precision
...
Delete from t_cb_mod_profit_usd_st;
...
--ALTER FUNCTION q_cb_mod_profit_usd_mt()
--    OWNER TO ictasadmin;
