-- FUNCTION: public.q_cb_mod_inc_usd_mt(text, text)

DROP FUNCTION IF EXISTS public.q_cb_mod_inc_usd_mt(text, text);

CREATE OR REPLACE FUNCTION public.q_cb_mod_inc_usd_mt(
	_user_id text,
	_session_id text)
    RETURNS boolean
    LANGUAGE 'plpgsql'
...
CREATE TABLE IF NOT EXISTS t_cb_mod_inc_usd_st
(
...
EXECUTE format('Insert into t_cb_mod_inc_usd_st

Select 
q_cb_mod_inc_usd.user_id, 
q_cb_mod_inc_usd.session_id, 
...
where q_cb_mod_inc_usd.user_id = %L and q_cb_mod_inc_usd.session_id = %L
group by 
q_cb_mod_inc_usd.user_id, 
q_cb_mod_inc_usd.session_id,
q_cb_mod_inc_usd.rep_month, 
...
q_cb_mod_inc_usd.l_6, 
c6_code.desc_tr_l6, 
q_cb_mod_inc_usd.key_r_pc_l6;', _user_id, _session_id);
...
--ALTER FUNCTION public.q_cb_mod_inc_usd_mt(text, text)
    --OWNER TO ictasadmin;
