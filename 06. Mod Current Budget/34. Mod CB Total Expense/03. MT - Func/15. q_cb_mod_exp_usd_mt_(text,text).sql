-- FUNCTION: public.q_cb_mod_exp_usd_mt(text, text)

DROP FUNCTION IF EXISTS public.q_cb_mod_exp_usd_mt(text, text);

CREATE OR REPLACE FUNCTION public.q_cb_mod_exp_usd_mt(
	_user_id text,
	_session_id text)
    RETURNS boolean
    LANGUAGE 'plpgsql'
...
CREATE TABLE IF NOT EXISTS t_cb_mod_exp_usd_st
(
...
EXECUTE format('Insert into t_cb_mod_exp_usd_st
select q_cb_mod_exp_usd.user_id,
q_cb_mod_exp_usd.session_id,
...
where q_cb_mod_exp_usd.user_id = %L and q_cb_mod_exp_usd.session_id = %L 
group by q_cb_mod_exp_usd.user_id,q_cb_mod_exp_usd.session_id,q_cb_mod_exp_usd.rep_month, q_cb_mod_exp_usd.pc, l_1 || ''.'' || l_2 || ''.'' || l_3 || ''.'' || l_4 || ''.'' || l_5 || ''.'' || l_6, c2_code.desc_tr_l2, c3_code.desc_tr_l3, q_cb_mod_exp_usd.month, q_cb_mod_exp_usd.up_curr_conv, q_cb_mod_exp_usd.l_1, q_cb_mod_exp_usd.l_2, q_cb_mod_exp_usd.l_3, q_cb_mod_exp_usd.l_4, q_cb_mod_exp_usd.l_5, q_cb_mod_exp_usd.l_6, q_cb_mod_exp_usd.key_r_pc_l6;',_user_id,_session_id);
...
--ALTER FUNCTION public.q_cb_mod_exp_usd_mt(text, text)
    --OWNER TO ictasadmin;
