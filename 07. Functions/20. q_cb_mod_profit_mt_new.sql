-- FUNCTION: public.q_cb_mod_profit_mt(text, text)

DROP FUNCTION IF EXISTS public.q_cb_mod_profit_mt(text, text);
...
AS $BODY$
    ...
    CREATE TABLE IF NOT EXISTS t_cb_mod_profit_st
    (
...
CREATE OR REPLACE VIEW q_cb_mod_pr_wbs
    AS
    ...
    GROUP BY t_cb_mod_exp_st.user_id,t_cb_mod_exp_st.session_id,t_cb_mod_exp_st.rep_month, t_cb_mod_exp_st.pc, t_cb_mod_exp_st.l_1, t_cb_mod_exp_st.l_2, t_cb_mod_exp_st.l_3, t_cb_mod_exp_st.l_4, t_cb_mod_exp_st.l_5, t_cb_mod_exp_st.l_6, t_cb_mod_exp_st.curr, t_cb_mod_exp_st.month, t_cb_mod_exp_st.key_r_pc_l6
    ...
    CREATE OR REPLACE VIEW q_cb_mod_profit
    AS
    ...
    FROM q_cb_mod_pr_wbs
        LEFT JOIN t_cb_mod_inc_st ON q_cb_mod_pr_wbs.key_r_pc_l6::text = t_cb_mod_inc_st.key_r_pc_l6::text AND q_cb_mod_pr_wbs.month = t_cb_mod_inc_st.inc_base_mon AND q_cb_mod_pr_wbs.curr::text = t_cb_mod_inc_st.curr::text AND q_cb_mod_pr_wbs.user_id::text = t_cb_mod_inc_st.user_id::text AND q_cb_mod_pr_wbs.session_id::text = t_cb_mod_inc_st.session_id::text
        LEFT JOIN t_cb_mod_exp_st ON q_cb_mod_pr_wbs.key_r_pc_l6::text = t_cb_mod_exp_st.key_r_pc_l6::text AND q_cb_mod_pr_wbs.month = t_cb_mod_exp_st.month AND q_cb_mod_pr_wbs.curr::text = t_cb_mod_exp_st.curr::text AND q_cb_mod_pr_wbs.user_id::text = t_cb_mod_exp_st.user_id::text AND q_cb_mod_pr_wbs.session_id::text = t_cb_mod_exp_st.session_id::text ,
        LATERAL ( SELECT
                    CASE
  ...
  EXECUTE format('Delete from t_cb_mod_profit_st where user_id = %L and session_id = %L;', _user_id, _session_id);  
    ...
    select 
	q_cb_mod_profit.user_id, 
	q_cb_mod_profit.session_id, 
